...
Provides phone formatting, callback URL generation, and HMAC-SHA256 signature helpers.
Python 3.12 compatible. No Odoo imports — pure stdlib + no circular risk.
"""
...
def get_at_status_callback_url(company, uuid):
    ...
    """
    base_url = company.get_base_url().rstrip('/')
    return f'{base_url}/sms_africastalking/status/{uuid}'
...
def generate_at_callback_signature(company, sms_uuid, callback_params):
    ...
    Args:
        company:         res.company record (must have at_api_key field).
        sms_uuid:        str — 32-char hex UUID embedded in the callback URL.
        callback_params: dict — POST form data from the AT webhook request.

    Returns:
        str — base64-encoded HMAC-SHA256 signature, or '' if api_key is absent.
    """
    ...
    digest = hmac.new(
        company.at_api_key.encode('utf-8'),
        data,
        hashlib.sha256,
    ).digest()
...
def validate_at_delivery_status(at_status):
    """
    Map an Africa's Talking delivery status string to an Odoo sms.tracker state.

    Returns one of: 'outgoing' | 'sent' | 'error'
    """
    _AT_TO_ODOO = {
        'Success':   'sent',
        'Sent':      'sent',
